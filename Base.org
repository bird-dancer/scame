...
when inserting a new link to a heading in the current document, an auto completion mini-buffer will pop up, suggesting all headings in the current document. From: [[https://howardism.org/Technical/Emacs/tiddly-wiki-in-org.html][TiddlyWiki In Org]]
#+begin_src emacs-lisp
  ;;;###autoload
...
:END:
** dired search filenames
isearch searches for only filenames in dired when cursor is on filename
#+begin_src emacs-lisp
...
#+end_src
** use tramp specific history
#+begin_src emacs-lisp :tangle no
  (setq tramp-remote-process-environment
        (cons "HISTFILE=~/.tramp_history"
              (delete "HISTFILE=/dev/null" tramp-remote-process-environment)))
#+end_src
** which-key mode
...
#+begin_src emacs-lisp
  (which-key-mode t)
#+end_src
** save emacs command to kill ring
*** evaluate and copy
#+begin_src emacs-lisp
...
** save shell command to kill ring
#+begin_src emacs-lisp
  ;;;###autoload
...
* hexl
:PROPERTIES:
:header-args: :tangle base/hexl.el :results silent :mkdirp yes
:END:
** improve address jumping
*** idk
#+begin_src emacs-lisp
  ;;;###autoload
...
#+begin_src emacs-lisp :tangle no
  ...
  (defun scame/hexl-goto-hex-address (expr)
   ...
   Examples:
     ...
     +0x10      → move forward 0x10 bytes
     -32        → move back 32 bytes"
    (interactive "shexAddress (expression): ")
    (require 'calc)
    (let* ((cur (hexl-current-address))
           (relative (string-match-p "\\`[+-]" expr))
           ;; remove 0x
           (expr (replace-regexp-in-string
  		"0x[0-9A-Fa-f]+"
  		(lambda (s) (format "(%d)" (string-to-number (substring s 2))))
  		expr))
           (val (string-to-number (calc-eval expr) 16))
           (addr (if relative (+ cur val) val)))
      (hexl-goto-address addr)))
  (with-eval-after-load 'hexl
    (define-key hexl-mode-map (kbd "M-g") 'scame/hexl-goto-hex-address))
#+end_src
...
* eshell
:PROPERTIES:
:header-args: :tangle base/eshell-settings.el :results silent :mkdirp yes
:END:
** shell-command: respect shell specific settings via .rc file and use aliases
eg. =~/.bashrc=
#+begin_src emacs-lisp :tangle no
...
#+end_src
** eshell: use .bashrc fro aliases
from: [[https://www.emacswiki.org/emacs/EshellAlias][EmacsWiki: Eshell Alias]]
needs to be run everytime .shellrc file is updated
#+begin_src emacs-lisp :tangle no
  ;;;###autoload
